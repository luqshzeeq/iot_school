...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    // Assuming index.php is at the root of your 'main' directory (DocumentRoot)
...
// If form submission is POST but you want to retain GET filters, ensure they are passed or re-read from GET
// The current code primarily uses GET for filtering, which is fine.
...
// This function now expects $base_url to be an absolute path like "/admin_manage_users.php"
...
// Use absolute path for redirects. Since 'main' is your DocumentRoot, '/admin_manage_users.php' is correct.
        ...
        if ($check_stmt) {
            ...
            if ($check_result->num_rows > 0) {
                ...
                if ($stmt) {
                    ...
                    if ($stmt->execute()) {
                        $_SESSION['success_message'] = ($stmt->affected_rows > 0) ? "Teacher details updated successfully." : "No changes were made to the teacher details.";
                    } else {
            ...
            $_SESSION['error_message'] = "Error preparing uniqueness check: " . $conn->error;
        }
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_user'])) {
    ...
    if ($user_id_to_delete === false) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $_SESSION['success_message'] = ($stmt->affected_rows > 0) ? "Teacher deleted successfully." : "Teacher not found or already deleted.";
            } else {
...
// This part populates the edit form
    ...
    if ($user_to_edit_id_from_get !== false) {
        ...
        if ($stmt) {
            ...
            if ($row_edit = $result_edit->fetch_assoc()) {
                ...
                // Redirect immediately if user to edit not found, to prevent showing empty/broken edit form
...
$sql .= " ORDER BY id ASC"; // Or username, created_at etc.

$stmt_all = $conn->prepare($sql); 
$result_all_teachers = false; // Initialize
...
if ($stmt_all) {
    ...
    if ($stmt_all->execute()) {
        ...
        // Display error message if query fails, but don't unset session messages
        $error_message = "Error fetching teachers list: " . $stmt_all->error; 
    }
    $stmt_all->close();
} else {
    $error_message = "Error preparing statement to fetch teachers: " . $conn->error;
}
...
<head>
    ...
    <style>
        ...
        /* Standard input style from your original code */
        .input-std { 
        ...
        .input-std:focus {
             /* Tailwind's focus ring variables, or direct values */
            --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);
            --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(1px + var(--tw-ring-offset-width)) var(--tw-ring-color);
            box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000);
            border-color: #4f46e5; /* Example focus border color (indigo-600) */
        }
    </style>
</head>
...
<div class="flex h-screen overflow-hidden">
    <?php include 'sidebar.php'; // Ensure sidebar.php is in the same directory or adjust path ?>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white shadow-md">
             <div class="container mx-auto px-6 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button class="block h-10 w-10 rounded-full overflow-hidden border-2 focus:outline-none focus:border-indigo-500">
                        <img class="h-full w-full object-cover" src="https://placehold.co/40x40/718096/E2E8F0?text=<?php echo strtoupper(substr($_SESSION['username'] ?? 'A', 0, 1)); ?>" alt="Avatar" />
                    </button>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
            <div class="container mx-auto">
                ...
                <?php if ($error_message && !$user_to_edit_id): /* Only show general errors if not in edit mode, edit form will show its own if any */ ?>
                <div id="errorMessage" class="flash-message flash-error"><div class="flex"><i class="fas fa-exclamation-triangle fa-lg mr-3 py-1"></i><div><p class="font-bold">Error</p><p class="text-sm"><?php echo htmlspecialchars($error_message); ?></p></div></div></div>
                <?php endif; ?>
                                    ...
                                    <?php while ($teacher = $result_all_teachers->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                ...
                                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete the teacher \'<?php echo addslashes(htmlspecialchars($teacher['username'])); ?>\'? This action cannot be undone.');">
                                                    <input type="hidden" name="user_id_to_delete" value="<?php echo $teacher['id']; ?>">
                                                    <input type="hidden" name="search_query" value="<?php echo htmlspecialchars($search_query); ?>">
                                                    <input type="hidden" name="status_filter" value="<?php echo htmlspecialchars($status_filter); ?>">
                                                    <button type="submit" name="delete_user" class="text-red-600 hover:text-red-900"><i class="fas fa-trash-alt mr-1"></i>Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const sidebarTexts = document.querySelectorAll('.sidebar-text'); // Assuming these classes exist in sidebar.php

        function toggleSidebarDesktop() {
            if (!sidebar || !sidebarToggle) return;
            const toggleIcon = sidebarToggle.querySelector('i');
            sidebar.classList.toggle('w-64'); // Tailwind class for width
            sidebar.classList.toggle('w-20'); // Tailwind class for collapsed width
        ...
        if (sidebarToggle) {
            ...
            // toggleSidebarDesktop(); // Call once to set initial icon state if needed
        }

        if (mobileSidebarToggle && sidebar) {
            // Ensure sidebar starts hidden on mobile and has fixed positioning classes
        ...
        // If editing, scroll to edit section
